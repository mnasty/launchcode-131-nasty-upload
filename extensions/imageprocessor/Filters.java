...
public class Filters {
	...
	public static Color purplish(Color c) {
		int colormod = c.getGreen();
		int red = c.getRed();
		int blue = c.getBlue();
		int modified = (colormod - colormod);
		Color nopurple = new Color(red, modified, blue);
		return nopurple;
	...
	public static int posterize(int a) {
		int polarized = (a <= 127) ? 0 : 255;
		//int polarized2 = a / 128 * 255; ???
		return polarized ;   // FIXME
	}
	...
	public static Color grayscale(Color c) {
		...
		int averaged = red + green + blue / 3;
		Color grey = new Color(averaged, averaged, averaged); 
        return grey;
